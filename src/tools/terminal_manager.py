...
class TerminalManager:
    ...
    MACOS_TERMINALS = ["iTerm", "Terminal"]
    ...
    def open_terminal(
        ...
        try:
            if system == "Darwin":
                return self._open_macos_terminal(command, work_dir)
            elif system == "Windows":
                return self._open_windows_terminal(command, work_dir)
            else:
    ...
    def _open_macos_terminal(
        self, command: Optional[str], cwd: Optional[str]
    ) -> bool:
        """Открывает терминал на macOS."""
        try:
            if command:
            ...
            logger.error(f"macOS terminal error: {e}")
            return False

    def _open_windows_terminal(
        self, command: Optional[str], cwd: Optional[str]
    ) -> bool:
        """Открывает терминал на Windows."""
        try:
            if command:
                subprocess.Popen(
    ...
    def run_in_new_terminal(
        ...
        try:
            if system == "Linux":
                ...
                return self._open_macos_terminal(command, None)
            elif system == "Windows":
                return self._open_windows_terminal(command, None)
        except Exception as e:
