...
class NetworkTool:
    ...
    def ping(self, host: str, count: int = 4) -> dict[str, Any]:
        ...
        try:
            ...
            # Linux/Mac: rtt min/avg/max/mdev = 0.1/0.2/0.3/0.0 ms
            ...
            # Windows: Minimum = 1ms, Maximum = 3ms, Average = 2ms
                ...
                m_win = re.search(
                    r"(?i)Minimum[^\d]+(\d+)ms[^A]+Maximum[^\d]+(\d+)ms[^A]+Average[^\d]+(\d+)ms",
                    output,
                )
                if m_win:
            ...
            if m_loss:
                ...
                m_loss_win = re.search(r"\((\d+)%\s*loss\)", output)
                if m_loss_win:
                    loss_pct = float(m_loss_win.group(1))
