...
class CLIAssistantApp(App):
    ...
    BINDINGS = [
        ...
        Binding("ctrl+y", "toggle_yolo", "Режим"),
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="input-area"):
            with Horizontal(id="input-row"):
                ...
                # Кнопка переключения режима безопасности (🛡 SAFE / 🚀 YOLO).
                # Стоит между полем ввода и кнопкой Send — чтобы быть всегда
                # под рукой и при этом не мешать визуальному центру тяжести
                # (Send-кнопка остаётся в правом нижнем углу).
                yield Button("🛡", id="btn-mode", variant="default")
    ...
    async def on_mount(self) -> None:
        ...
        # Синхронизируем кнопку режима безопасности с конфигом
        self._refresh_mode_button()
        # Запускаем периодическое обновление системной информации (каждые 3с)
        self.set_interval(3.0, self._refresh_system_info)
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        """Обрабатывает кнопки нижней панели (отправка / переключение режима)."""
    ...
    async def _handle_slash_command(self, text: str) -> None:
        ...
        if cmd == "/help":
        ...
        elif cmd in ("/yolo", "/safe", "/mode"):
            # /yolo — включить YOLO без вопросов
            # /safe — включить SAFE-режим
            # /mode — toggle (или "/mode yolo" / "/mode safe")
            ...
            if cmd == "/yolo":
                ...
                a = arg.strip().lower()
                if a in ("yolo", "on", "1", "true"):
                    target = True
                elif a in ("safe", "off", "0", "false"):
                    target = False
                else:
                    target = None  # toggle
            await self.action_toggle_yolo(target)
    ...
    # ─── Режим безопасности (SAFE / YOLO) ────────────────────────────────────

    def _refresh_mode_button(self) -> None:
        """Синхронизирует подпись и стиль кнопки режима с текущим конфигом.

        SAFE-режим (по умолчанию) — иконка щита 🛡, нейтральная подсветка.
        YOLO-режим — иконка ракеты 🚀, красная подсветка (variant="error"),
        чтобы пользователь точно видел, что вопросов больше задавать не будут.
        """
        try:
            ...
            return
        try:
            yolo = bool(getattr(self._config.safety, "yolo_mode", False))
        except Exception:
            yolo = False
        if yolo:
            btn.label = "🚀"
            btn.variant = "error"
            btn.tooltip = "YOLO: подтверждения отключены (Ctrl+Y или /safe)"
        else:
            btn.label = "🛡"
            btn.variant = "default"
            btn.tooltip = "SAFE: подтверждения включены (Ctrl+Y или /yolo)"

    async def action_toggle_yolo(self, target: Optional[bool] = None) -> None:
        """Переключает YOLO/SAFE-режим.

        Параметр `target`:
          • None — toggle (переключение относительно текущего состояния);
          • True — принудительно YOLO;
          • False — принудительно SAFE.

        Изменение сохраняется в config.json. Хард-блок катастрофических
        команд (rm -rf /, dd of=/dev/sdX и т.п.) при этом ВСЕГДА остаётся
        активен — он не зависит от этого флага.
        """
        try:
            current = bool(getattr(self._config.safety, "yolo_mode", False))
        except Exception:
            current = False
        new_value = (not current) if target is None else bool(target)
        try:
            self._config.safety.yolo_mode = new_value
            get_config_manager().save()
            # SafetyChecker внутри ассистента смотрит на тот же объект
            # AppConfig — отдельная синхронизация ему не нужна, но reload
            # делаем на случай, если кто-то держит копию.
            try:
                self.assistant.reload_config()
            except Exception:
                pass
        ...
        try:
            ...
            if new_value:
                msg = (
                    "[bold red]🚀 YOLO-режим включён[/bold red]\n"
                    "[yellow]Подтверждения для деструктивных операций и sudo "
                    "БОЛЬШЕ НЕ ЗАПРАШИВАЮТСЯ.[/yellow]\n"
                    "[dim]Хард-блок катастрофических команд (rm -rf /, dd of=/dev/* "
                    "и т.п.) остаётся активен. Выключить: Ctrl+Y или /safe[/dim]"
                )
            else:
                msg = (
                    "[bold green]🛡 SAFE-режим включён[/bold green]\n"
                    "[dim]Деструктивные действия и sudo требуют подтверждения. "
                    "Выключить: Ctrl+Y или /yolo[/dim]"
                )
            await chat_area.mount(Static(msg))
            chat_area.scroll_end(animate=False)
        except Exception:
            pass
    ...
    /* Ширину Select-профиля задаём ровно как контейнеру; внутреннюю
       разметку Textual (SelectCurrent / Static с подписью) НЕ трогаем —
       любые `height: 1` на её внутренностях ломают отрисовку и текст
...
[bold]Slash-команды:[/bold]
  ...
  [cyan]/cd[/cyan]                — сменить рабочую директорию
  [cyan]/ls[/cyan]                — содержимое текущей директории
  [cyan]/sudo[/cyan]              — запросить sudo пароль
  [cyan]/sudo clear[/cyan]        — очистить кэш sudo

[bold]Режим безопасности:[/bold]
  [cyan]/yolo[/cyan]              — 🚀 YOLO: выполнять всё без подтверждений
  [cyan]/safe[/cyan]              — 🛡 SAFE: спрашивать подтверждение (по умолчанию)
  [cyan]/mode \\[yolo|safe][/cyan]  — переключить или установить режим
  [dim]Также: кнопка 🛡/🚀 рядом с полем ввода или Ctrl+Y.[/dim]
  [dim]Хард-блок катастрофических команд (rm -rf /, dd of=/dev/* и т.п.) активен ВСЕГДА.[/dim]
